  ...
  list: [
    {
      ...
      descriptions: [
        ...
        { content: '사용자 수의 증가에 따른 초과 수량 및 시간에 따른 상품 배정, 초과 수량 조절을 위한 템플릿 개발 프로젝트' },
        { content: '이벤트 번호와 시간 설정, 갯수 제한을 통한 이벤트 제한 템플릿 기능 개발' },
        { content: '@Transactional(isolation = Isolation.READ_UNCOMMITTED)을 사용하여 Dirty Read이지만,\n' +
            '많은 유저 동시 신청시 이전의 결과가 COMMIT 되지 않아도 합산되게 하여 초과 수량 신청 방지' },
        { content: 'Junit4ClassRunner 및 standaloneSetup, @Spy 객체 등을 활용하여, 입력값은 이벤트 번호만 입력하면, 템플릿화 한 신청 양식을 통해 자동으로 신청까지 테스트할 수 있는 테스트 기능 개발' },
      ],
    },
    {
      ...
      descriptions: [
        ...
        { content: 'Axios를 이용하여 Spring, React 간의 요청사항을 처리하였고, 서버에서 필요한 값을 불러올때, 해당 값 검증 후 진행을 하여야 해서 async - await를 사용' },
        { content: '추후에 발생되는 Store 객체 및 Axios를 이용한 API 등에 대해 어떤 기능일지 확인 및 적용이 필요하여,해당 부분에 대해 작성 및 수정이 가능한 내부 URL Page 구축' },
      ],
    },
    {
      title: '수기계좌 정산 기능 구축',
      ...
      descriptions: [
        ...
        { content: '기존에 Excel로 관리되었던 수기 정산을 Admin System을 통한 등록 및 자동 정산 계산 기능 개발' },
